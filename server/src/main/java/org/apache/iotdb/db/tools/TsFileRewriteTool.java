...
public class TsFileRewriteTool implements AutoCloseable { // TsFile重写工具
  ...
  protected Map<TsFileIOWriter, ModificationFile>
      fileModificationMap; // 当旧TsFile有删除记录时，则该列表存放着升级后对应每个新TsFileIOWriter类对象都有自己对应新TsFile文件的新ModificationFile(.mods)修改文件类对象；否则该列表为空
  ...
  public TsFileRewriteTool(TsFileResource resourceToBeRewritten) throws IOException {
    ...
    if (FSFactoryProducer.getFSFactory()
        ...
        .exists()) { // 若本地存在该旧TsFile对应的.mods删除文件
      oldModification =
          (List<Modification>)
              resourceToBeRewritten.getModFile().getModifications(); // 将旧TsFile对应的一条条删除记录操作放入该修改列表里
        ...
        switch (marker) {
          ...
          case MetaMarker.ONLY_ONE_PAGE_CHUNK_HEADER:
            ...
            while (dataSize > 0) {
              ...
              ByteBuffer pageData =
                  !needToDecode
                      ? reader.readCompressedPage(
                          pageHeader) // 若该page数据不需要解码，则从当前位置直接读取经压缩后page大小的字节的数据到ByteBuffer缓存里。读指针向后移动
                      : reader.readPage(
                          pageHeader,
                          header
                              .getCompressionType()); // 若需要解码，则需要先把该page的二进制流数据进行解压uncompress，然后才能解码。即则根据pageHeader和压缩类型，把该page的数据进行解压后存入ByteBuffer缓存并返回
          ...
          case MetaMarker.OPERATION_INDEX_RANGE:
            ...
            for (TsFileIOWriter tsFileIOWriter : partitionWriterMap.values()) {
              ...
              tsFileIOWriter
                  .writePlanIndices(); // 往该TsFile对应的TsFileIOWriter类对象的TsFileOutput对象的输出流BufferedOutputStream的缓存数组里写入planIndex，并将该TsFileIOWriter类对象的TsFileOutput对象的输出流BufferedOutputStream的缓存数组内容flush到本地TsFile文件
            }
            break;
          default:
  ...
  protected boolean checkIfNeedToDecode( // 如果该Chunk只有一个page或者该page的数据不在一个时间分区里则需要解码
    ...
    if (pageHeader.getStatistics()
        == null) { // 若该Chunk只有一个page，则该page的pageHeader没有statistics信息，这样就不能知道该page数据的起使时间和结束时间，进而无法判断该page数据所属的时间分区
      return true;
    }
    // Decode is required if the page has data to be deleted. Otherwise, decode is not required
    if (oldModification
        != null) { // 若该page的数据存在待删除的部分则需要反序列化解码decode,否则则不需要（可以直接以二进制流的形式写入新TsFile里）
      modsIterator = oldModification.iterator();
   ...
   */
  protected void reWriteChunk( // 重写该Chunk的数据到对应时间分区的TsFile文件里：(1)
      // 先遍历该Chunk的每个page的pageData，将该page的每个数据点写入对应Chunk的ChunkWriterImpl的输出流pageBuffer缓存里
      // (2)遍历每个时间分区TsFile对应的待写入传感器Chunk的ChunkWriterImpl对象,使用该Chunk的ChunkWriterImpl对象往对应TsFile的TsFileIOWriter的TsFileOutput写入对象的输出流BufferedOutputStream的缓存数组里写入该Chunk的内容（ChunkHeader和page内容（pageHeader+pageData）），若是第一个Chunk则要先往对应的TsFileIOWriter的TsFileOutput写入对象的输出流BufferedOutputStream的缓存数组里写入ChunkGroupHeader
      String deviceId,
      boolean firstChunkInChunkGroup, // 该Chunk是否是所属ChunkGroup里的第一个Chunk
      UnaryMeasurementSchema schema, // 传感器配置类对象
      ...
      long chunkHeaderOffset) // 该Chunk的所有page里的数据是否需要解码列表
      throws IOException, PageException { // 此方法是以Chunk里的page为单位进行重写（可以自己另创方法考虑以chunk为单位）
    valueDecoder =
        Decoder.getDecoderByType(
            schema.getEncodingType(), schema.getType()); // 根据该Chunk传感器的编码方式和数据类型获得对应的解码器对象
    Map<Long, ChunkWriterImpl> partitionChunkWriterMap =
        new HashMap<>(); // 存放每个时间分区TsFile的某一传感器对应的ChunkWriterImpl类对象
    for (int i = 0;
        i < pageDataInChunk.size();
        i++) { // 遍历每个page，将该page的每个数据点写入其所属时间分区TsFile的对应Chunk的ChunkWriterImpl的输出流pageBuffer缓存里
      if (Boolean.TRUE.equals(
          needToDecodeInfoInChunk.get(
              i))) { // 若该page数据需要解码，则说明需要把二进制流形式的pageData反序列化成数据点对象类，即放入BatchData类对象里
        decodeAndWritePage(
            deviceId,
            schema,
            pageDataInChunk.get(i),
            partitionChunkWriterMap,
            chunkHeaderOffset); // 将给定存放在二进制缓存里的pageData进行解码成BatchData类对象，并对里面的每个数据点重写进TsFile里。即将每个数据点所属的时间分区和其所属的传感器的schema配置对象创建每个时间分区TsFile的该传感器对应的ChunkWriterImpl对象，用对应的ChunkWriterImpl把每个数据点交由该Chunk的pageWriter写入到其对应的两个输出流timeOut和valueOut的缓存中，并检查该Chunk的pageWriter的数据点or占用内存的大小情况，判断是否要开启一个新的page
      } else { // 若该page数据不需要解码，则直接把二进制流形式的pageData写进其所属Chunk的ChunkWriterImpl的输出流pageBuffer缓存里
        writePage(
            schema,
            pageHeadersInChunk.get(i),
            pageDataInChunk.get(i),
            partitionChunkWriterMap); // 此处直接根据给定的pageHeader和pageData，根据该page数据所在时间分区和该page所属的传感器创建对应的ChunkWriterImpl对象，并用该ChunkWriterImpl对象把该页page数据（pageData+pageHeader）写入该Chunk的ChunkWriterImpl的输出流pageBuffer缓存里
        // 该page数据是在一个时间分区里的
      }
    }
    for (Entry<Long, ChunkWriterImpl> entry :
        partitionChunkWriterMap
            .entrySet()) { // 遍历每个时间分区TsFile对应的待写入传感器Chunk的ChunkWriterImpl对象,使用该Chunk的ChunkWriterImpl对象往对应TsFile的TsFileIOWriter的TsFileOutput写入对象的输出流BufferedOutputStream的缓存数组里写入该Chunk的内容（ChunkHeader和page内容（pageHeader+pageData）），若是第一个Chunk则要先往对应的TsFileIOWriter的TsFileOutput写入对象的输出流BufferedOutputStream的缓存数组里写入ChunkGroupHeader
      long partitionId = entry.getKey(); // 获取时间分区ID
      TsFileIOWriter tsFileIOWriter =
          partitionWriterMap.get(partitionId); // 根据时间分区ID获取对应TSFile文件的TsFileIOWriter类对象
      if (firstChunkInChunkGroup
          || !tsFileIOWriter.isWritingChunkGroup()) { // 如果是第一个Chunk，则写入ChunkGroupHeader
        tsFileIOWriter.startChunkGroup(
            deviceId); // 开启一个新的设备ChunkGroup，即把该新的ChunkGroupHeader的内容（marker（为0）和设备ID）写入该TsFileIOWriter对象的TsFileOutput写入对象的输出流BufferedOutputStream的缓存数组里
      }
      // write chunks to their own upgraded tsFiles
      IChunkWriter chunkWriter =
          entry.getValue(); // 该时间分区的TsFile文件有了该设备的ChunkGroup后，就获取ChunkWriterImpl对象
      chunkWriter.writeToFileWriter(
          tsFileIOWriter); // 使用ChunkWriterImpl对象，首先封口当前page(即把当前Chunk的pageWriter输出流timeOut和valueOut的缓存数据写到该Chunk的ChunkWriterImpl的输出流pageBuffer缓存里,最后重置该pageWriter)，然后往TsFileIOWriter对象的TsFileOutput写入对象的输出流BufferedOutputStream的缓存数组里写入该Chunk的ChunkHeader,最后再写入当前Chunk的所有page数据（pageBuffer输出流的缓存数组内容）
    }
  }

  protected void endChunkGroup()
      throws
          IOException { // 结束每个时间分区的TsFileIOWriter类对象对应ChunkGroup，即把其ChunkGroupMeatadata元数据加入该写入类的chunkGroupMetadataList列表缓存里，并把该写入类的相关属性（设备ID和ChunkMetadataList清空）
    for (TsFileIOWriter tsFileIoWriter :
        partitionWriterMap.values()) { // 遍历获取每个时间分区的TsFileIOWriter类对象
      tsFileIoWriter
          .endChunkGroup(); // 结束该TsFileIOWriter类对象对应ChunkGroup，即把其ChunkGroupMeatadata元数据加入该写入类的chunkGroupMetadataList列表缓存里，并把该写入类的相关属性（设备ID和ChunkMetadataList清空），并将该TsFile的TsFileIOWriter对象的输出缓存流TsFileOutput的内容给flush到本地对应TsFile文件
    }
  }
  ...
  protected TsFileIOWriter getOrDefaultTsFileIOWriter(
      File oldTsFile,
      long
          partition) { // 获取指定时间分区partition对应的TsFileIOWriter类对象，若没有则：（1）在本地新建一个该时间分区目录下的TsFile文件（2）然后用该新TsFile文件创建对应新的TsFileIOWriter类对象（3）若原先旧TsFile存在删除操作，则创建该新TsFile对应的修改文件ModificationFile类对象，并往fileModificationMap放入该TsFile的TsFileIOWriter和对应的新修改文件ModificationFile对象
    return partitionWriterMap
        .computeIfAbsent( // 获取指定时间分区partition对应的TsFileIOWriter类对象，若没有则在本地新建一个该时间分区目录下的TsFile文件，然后用该新TsFile文件创建对应新的TsFileIOWriter类对象
            partition,
  ...
  protected void
    ...
    long partitionId =
        StorageEngine.getTimePartition(pageHeader.getStartTime()); // 获取此page开始时间所属的时间分区
    getOrDefaultTsFileIOWriter(
        oldTsFile,
        partitionId); // 获取指定时间分区partition对应的TsFileIOWriter类对象，若没有则：（1）在本地新建一个该时间分区目录下的TsFile文件（2）然后用该新TsFile文件创建对应新的TsFileIOWriter类对象（3）创建该新TsFile对应的修改文件ModificationFile类对象，并往fileModificationMap放入该TsFile的TsFileIOWriter和对应的新修改文件ModificationFile对象
    ChunkWriterImpl chunkWriter =
        partitionChunkWriterMap.computeIfAbsent(
            partitionId,
            v ->
                new ChunkWriterImpl(schema)); // 获取该page经更新分割后所在时间分区TsFile里的Chunk传感器的ChunkWriterImpl
    chunkWriter.writePageHeaderAndDataIntoBuff(
        pageData,
        pageHeader); // 往当前Chunk的ChunkWriterImpl对象的输出流pageBuffer里写入page数据（pageData+pageHeader，此处pageData是存放在二进制缓存ByteBuffer对象里）
  }

  protected void
      decodeAndWritePage( // 将给定存放在二进制缓存里的pageData进行解码成BatchData类对象，并对里面的每个数据点重写进TsFile里。即将每个数据点所属的时间分区和其所属的传感器的schema配置对象创建每个时间分区TsFile的该传感器对应的ChunkWriterImpl对象，用对应的ChunkWriterImpl把每个数据点交由该Chunk的pageWriter写入到其对应的两个输出流timeOut和valueOut的缓存中，并检查该Chunk的pageWriter的数据点or占用内存的大小情况，判断是否要开启一个新的page
          String deviceId,
          UnaryMeasurementSchema schema,
          ByteBuffer pageData,
          Map<Long, ChunkWriterImpl> partitionChunkWriterMap,
    ...
    BatchData batchData =
        pageReader
            .getAllSatisfiedPageData(); // 通过pageReader对象的时间解码器和数值解码器去读取该page所有满足条件（不在被删除时间范围内以及符合过滤器，此处删除时间范围是null）的时间戳和对应的数据，放入BatchData类对象里
    rewritePageIntoFiles(
        batchData,
        schema,
        partitionChunkWriterMap); // 该方法是将传来的page数据进行重写到TsFile里，根据传来batchData里每个数据点所属的时间分区和其所属的传感器的schema配置对象创建每个时间分区TsFile的该传感器对应的ChunkWriterImpl对象，用对应的ChunkWriterImpl把每个数据点交由该Chunk的pageWriter写入到其对应的两个输出流timeOut和valueOut的缓存中，并检查该Chunk的pageWriter的数据点or占用内存的大小情况，判断是否要开启一个新的page
  }

  /** Benson */
  protected void decodeAndWritePageInOnePartition(
      UnaryMeasurementSchema schema, ByteBuffer pageData, ChunkWriterImpl chunkWriter)
      throws IOException {
    valueDecoder.reset(); // 重置解码器
    PageReader pageReader = // 根据二进制流的pageData和数据类型，以及时间、数值解码器去创建Page读取类对象
        new PageReader(pageData, schema.getType(), valueDecoder, defaultTimeDecoder, null);
    ...
    BatchData batchData =
        pageReader
            .getAllSatisfiedPageData(); // 通过pageReader对象的时间解码器和数值解码器去读取该page所有满足条件（不在被删除时间范围内以及符合过滤器）的时间戳和对应的数据，放入BatchData类对象里
    rewritePageInOnePartitionIntoFiles(
        batchData, schema, chunkWriter); // 此处只是把该page数据顺序写到对应Chunk的ChunkWriterImpl的输出流pageBuffer缓存里
  }
  ...
  protected void
    ...
    while (batchData.hasCurrent()) {
      ...
      getOrDefaultTsFileIOWriter(
          oldTsFile,
          partitionId); // 获取指定时间分区partition对应的TsFileIOWriter类对象，若没有则：（1）在本地新建一个该时间分区目录下的TsFile文件（2）然后用该新TsFile文件创建对应新的TsFileIOWriter类对象（3）创建该新TsFile对应的修改文件ModificationFile类对象，并往该类对象的fileModificationMap属性放入该新TsFile的TsFileIOWriter和对应的新修改文件ModificationFile对象
      switch (schema.getType()) {
        case INT32:
          chunkWriter.write(
              time,
              (int) value,
              false); // 将给定的数据点交由该Chunk的pageWriter写入到其对应的两个输出流timeOut和valueOut的缓存中，并检查该Chunk的pageWriter的数据点or占用内存的大小情况，判断是否要开启一个新的page
          break;
        case INT64:
  ...
  protected void
      rewritePageIntoFiles( // 该方法是将传来的page数据进行重写到TsFile里，根据传来batchData里每个数据点所属的时间分区和其所属的传感器的schema配置对象创建每个时间分区TsFile的该传感器对应的ChunkWriterImpl对象，用对应的ChunkWriterImpl把每个数据点交由该Chunk的pageWriter写入到其对应的两个输出流timeOut和valueOut的缓存中，并检查该Chunk的pageWriter的数据点or占用内存的大小情况，判断是否要开启一个新的page，若要开启新的page则要把当前Chunk的pageWriter里两个输出流timeOut和valueOut的缓存数据写到该Chunk的ChunkWriterImpl的输出流pageBuffer缓存里。结束当前page的写操作后需要把当前pageWriter缓存里的数据flush写到所属Chunk的ChunkWriterImpl的输出流pageBuffer缓存里
          BatchData batchData, // 存放某page里经mods文件过滤后的data
          UnaryMeasurementSchema schema, // 该page所属Chunk传感器的配置类对象
          Map<Long, ChunkWriterImpl>
              partitionChunkWriterMap) { // 旧的page数据经update更新可能被分割成多个时间分区的数据。比如旧的page所在的Chunk对应的传感器ID为root.demo.s1.height，这个时候进行更新操作，该旧page数据被分割存储在时间分区0和1的两个TsFile里各自的root.demo.s1.height传感器对应Chunk里。即旧page数据可能被分割存储在不同时间分区TsFile的各自传感器Chunk里，于是经分割后不同时间分区的数据需要有自己的ChunkWriterImpl进行写入到对应时间分区的TsFile文件的传感器Chunk里
    while (batchData
        .hasCurrent()) { // 循环遍历指定page里的batchData的数据点，根据每个数据点时间戳所在的时间分区获取对应的ChunkWriterImpl类对象，
      long time = batchData.currentTime(); // 获取时间戳
      Object value = batchData.currentValue(); // 获取该时间戳对应的数据
      long partitionId =
          StorageEngine.getTimePartition(
              time); // 根据时间戳获取其所属时间分区ID（此处是更新操作，将旧数据按照时间戳划分所属时间分区以决定后续写入哪个TsFile文件里）

      ChunkWriterImpl
          chunkWriter = // 从partitionChunkWriterMap中拿取该时间分区对应的ChunkWriterImpl类对象，若不存在则用传感器配置类对象创建一个
          partitionChunkWriterMap.computeIfAbsent(
                  partitionId,
                  v ->
                      new ChunkWriterImpl(
                          schema)); // 经分割后不同时间分区的数据需要有自己的ChunkWriterImpl进行写入到对应时间分区的TsFile文件的传感器Chunk里
      getOrDefaultTsFileIOWriter(
          oldTsFile,
          partitionId); // 获取指定时间分区partition对应的TsFileIOWriter类对象，若没有则：（1）在本地新建一个该时间分区目录下的TsFile文件（2）然后用该新TsFile文件创建对应新的TsFileIOWriter类对象（3）创建该新TsFile对应的修改文件ModificationFile类对象，并往fileModificationMap放入该TsFile的TsFileIOWriter和对应的新修改文件ModificationFile对象
      switch (schema.getType()) {
    ...
    partitionChunkWriterMap
        ...
        .forEach(
            writer -> {
              writer
                  .sealCurrentPage(); // 强制把每个ChunkWriter的当前page那些还存留在pageWriter缓存的数据给强制flush写到对应Chunk的ChunkWriterImpl的输出流pageBuffer缓存里
            });
  }
  ...
  protected TsFileResource endFileAndGenerateResource(TsFileIOWriter tsFileIOWriter)
      throws
          IOException { // 该方法会做两件事：（1）在向该TsFileIOWriter的TsFileOutput的缓存输出流BufferedOutputStream的数组里写完数据区的内容并flush到本地后，再往该缓存里写对应索引区的内容，以及剩余的小内容（如TsFile结尾的Magic String等），最后关闭该TsFileIOWriter的TsFileOutput的两个输出流BufferedOutputStream和FileOutputStream，会往对应的本地TsFile写入该TsFileIOWriter缓存里的数据（即索引区和小内容等，数据区之前已经flush到本地文件了）（2）根据新的TsFile对象创建对应的TsFileResource对象，并把该TsFileResource对象的内容写到本地对应的.resource文件里
    tsFileIOWriter
        .endFile(); // 在向该TsFileIOWriter的TsFileOutput的缓存输出流BufferedOutputStream的数组里写完数据区的内容并flush到本地后，再往该缓存里写对应索引区的内容，以及剩余的小内容（如TsFile结尾的Magic String等），最后关闭该TsFileIOWriter的TsFileOutput的两个输出流BufferedOutputStream和FileOutputStream，会往对应的本地TsFile写入该TsFileIOWriter缓存里的数据（即索引区和小内容等，数据区之前已经flush到本地文件了）
    TsFileResource tsFileResource =
        new TsFileResource(tsFileIOWriter.getFile()); // 根据新建的TsFile文件对象创建对应的TsFileResource对象
    ...
    for (Entry<String, List<TimeseriesMetadata>> entry :
        deviceTimeseriesMetadataMap.entrySet()) { // 遍历每个设备和对应设备下所有传感器的TimeseriesMetadata对象列表
      String device = entry.getKey(); // 获取设备路径
      for (TimeseriesMetadata timeseriesMetaData :
          entry.getValue()) { // 遍历该设备下所有传感器的TimeseriesMetadata对象列表
        tsFileResource.updateStartTime(
            device,
            timeseriesMetaData.getStatistics().getStartTime()); // 更新TsFileResource的startTime
        tsFileResource.updateEndTime(
            device, timeseriesMetaData.getStatistics().getEndTime()); // 更新TsFileResource的endTime
      }
    }
