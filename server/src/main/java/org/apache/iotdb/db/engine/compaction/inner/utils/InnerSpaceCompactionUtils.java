...
public class InnerSpaceCompactionUtils {
  ...
  //3. 往目标文件writer的输出缓存写完数据区的内容并flush到本地后，往该缓存里写对应索引区的内容，以及剩余的小内容（如TsFile结尾的Magic String等），最后关闭该writer的输出流，会往对应的本地TsFile写入该TsFileIOWriter缓存里的数据（即索引区和小内容等，数据区之前已经flush到本地文件了）
