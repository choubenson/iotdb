...
public class UpgradeTask extends WrappedRunnable { // 升级线程类，每个升级线程类用于处理一个待升级的TsFile文件，并对他进行相关升级操作
  ...
  public void runMayThrow() { // 升级线程运行的内容
    try {
      ...
      if (!UpgradeUtils.isUpgradedFileGenerated(
          upgradeResource.getTsFile().getName())) { // 如果该TsFile的新的升级文件(.tsfile)没有生成、封口，则
        logger.info("generate upgraded file for {}", upgradeResource.getTsFile());
        upgradedResources =
            generateUpgradedFiles(); // 对旧的TsFile文件进行升级并生成新的TsFile文件和对应新的.resource文件和.mods修改文件，并返回升级完成后生成的一个or多个新TsFile的TsFileResource
      } else { // 若该TsFile的新的升级文件(.tsfile)已经生成并封口了
        logger.info("find upgraded file for {}", upgradeResource.getTsFile());
        upgradedResources =
            findUpgradedFiles(); // 该方法返回升级后的新TsFile对应的多个TsFileResource文件对象（因为数据的时间戳所在时间分区不同，导致升级后存在多个TsFile）
      }
      upgradeResource.setUpgradedResources(
          upgradedResources); // 设置该旧TsFileResource对应旧TsFile对应的升级完后的多个TsFile对应TsFileRewource。//对于该旧TsFileResource对应的TsFile文件升级后（从v0.11.x/v2 升级到0.12/v3）产生的新的多个TsFileResource存入该列表里。
  ...
  private List<TsFileResource> generateUpgradedFiles()
      throws IOException,
          WriteProcessException { // 对旧的TsFile文件进行升级并生成新的一个或多个时间分区的TsFile文件和对应新的.resource文件和.mods修改文件，并返回升级完成后生成的一个or多个新TsFile的TsFileResource
    ...
    try {
      TsFileOnlineUpgradeTool.upgradeOneTsFile(
          upgradeResource,
          upgradedResources); // 该方法用于升级一个TsFile文件，第一个参数是旧的待升级文件的TsFileResource对象，第二个参数是升级完毕新的TsFile文件TsFileResource列表。由于升级时可能会对旧的TsFile根据时间分区分成多个TsFile，因此第二个参数是列表
  ...
  private List<TsFileResource> findUpgradedFiles()
      throws
          IOException { // 该方法返回升级后的新TsFile对应的多个TsFileResource文件对象（因为数据的时间戳所在时间分区不同，导致升级后存在多个TsFile）
    upgradeResource.readLock();
    List<TsFileResource> upgradedResources = new ArrayList<>();
