...
public class TsFileWriter implements AutoCloseable {//每个TsFile对应的Writer
   ...
   */
  private boolean checkIsTimeSeriesExist(TSRecord record) throws WriteProcessException {//判断当前的TsRecord数据是否合法（即该TsFile里存在对应的时间序列），若合法则：（1）判断当前TsFileWriter是否有对应设备ID的ChunkGroupWriter，若没有则新建一个放入groupWriters里。（2）判断对应设备ID的chunkGroupWriter里是否存在对应传感器ID的ChunkWriter，若没有则新建一个放入chunkWriters里
    ...
    if (!groupWriters.containsKey(record.deviceId)) { //若groupWriters变量里不包含该设备，则
      groupWriter = new ChunkGroupWriterImpl(record.deviceId);//新建一个属于该设备ID的ChunkGroupWriterImpl，并把它放入groupWriters变量里
    ...
    for (DataPoint dp : record.dataPointList) { //遍历该TsRecord对象里在该时间戳上所有的传感器对应的数据
      ...
      if (schema.containsTimeseries(path)) {  //若当前TsFile的配置类对象schema里存在该时间序列，则
        groupWriter.tryToAddSeriesWriter(schema.getSeriesSchema(path), pageSize);//判断该ChunkGroupWriter里是否存在此传感器ID对应的ChunkWriter，若不存在，则创建一个
      } else if (schema.getSchemaTemplates() != null && schema.getSchemaTemplates().size() == 1) {//若当前TsFile的配置类对象schema里不存在该时间序列，且配置类对象schema的配置模板数量为1，则
        ...
        if (template.containsKey(path.getMeasurement())) {//若此次遍历到的配置模板包含该MeasureId，则
          groupWriter.tryToAddSeriesWriter(template.get(path.getMeasurement()), pageSize);//根据该配置模板获取对应传感器的传感器配置类对象，判断该ChunkGroupWriter里是否存在此传感器ID对应的ChunkWriter，若不存在，则创建一个
        }
      } else {
   ...
   * @throws WriteProcessException exception
   */
  private void checkIsTimeSeriesExist(Tablet tablet) throws WriteProcessException {//判断当前的Tablet数据是否合法（即该TsFile里存在对应的时间序列），若合法则：（1）判断当前TsFileWriter是否有对应设备ID的ChunkGroupWriter，若没有则新建一个放入groupWriters里。（2）判断对应设备ID的chunkGroupWriter里是否存在对应传感器ID的ChunkWriter，若没有则新建一个放入chunkWriters里
    IChunkGroupWriter groupWriter;
    if (!groupWriters.containsKey(tablet.prefixPath)) {//若groupWriters变量里不包含该设备，则
      groupWriter = new ChunkGroupWriterImpl(tablet.prefixPath);//新建一个属于该设备ID的ChunkGroupWriterImpl，并把它放入groupWriters变量里
   ...
   */
  public boolean write(TSRecord record) throws IOException, WriteProcessException {//写入TsRecord数据，粗略步骤为：（1）判断当前TsFile里是否存在给定TsRecord数据的时间序列（2）若第一步检查合法，则将TsRecord里的所有传感器的数据点写入对应ChunkWriter的PageWriter的缓存里。（3）最后要判断内存是否到达临界值而需要flush所有的ChunkGroups，若需要则将该TsFileWriter的所有ChunkGroupWriter里的所有ChunkWriter的缓存数据按序缓存到TsFileIOWriter的输出流缓存out里，最后将out输出流给flush到本地对应的TsFile文件。
    // make sure the ChunkGroupWriter for this TSRecord exist
    checkIsTimeSeriesExist(record);//判断当前的TsRecord数据是否合法（即该TsFile里存在对应的时间序列），若合法则：（1）判断当前TsFileWriter是否有对应设备ID的ChunkGroupWriter，若没有则新建一个放入groupWriters里。（2）判断对应设备ID的chunkGroupWriter里是否存在对应传感器ID的ChunkWriter，若没有则新建一个放入chunkWriters里
    // get corresponding ChunkGroupWriter and write this TSRecord
    groupWriters.get(record.deviceId).write(record.time, record.dataPointList);//将所有的数据点写入对应ChunkWriter的PageWriter的缓存里。具体做法是：遍历所有的data数据点（传感器ID，数值），根据每个数据点的传感器ID，将给定的数据点（time,value）交由该Chunk的pageWriter写入到其对应的两个输出流timeOut和valueOut的缓存中，并检查该Chunk的pageWriter的数据点or占用内存的大小情况，判断是否要开启一个新的page，若要开启新的page则往对应Chunk的ChunkWriterImpl的输出流pageBuffer缓存里写入该page的pageHeader和pageData（即pageWriter对象里输出流timeOut和valueOut的缓存数据），最后重置该pageWriter
  ...
  public boolean write(Tablet tablet) throws IOException, WriteProcessException {
    ...
    checkIsTimeSeriesExist(tablet);//判断当前的Tablet数据是否合法（即该TsFile里存在对应的时间序列），若合法则：（1）判断当前TsFileWriter是否有对应设备ID的ChunkGroupWriter，若没有则新建一个放入groupWriters里。（2）判断对应设备ID的chunkGroupWriter里是否存在对应传感器ID的ChunkWriter，若没有则新建一个放入chunkWriters里
    // get corresponding ChunkGroupWriter and write this Tablet
    groupWriters.get(tablet.prefixPath).write(tablet);//依次遍历tablet结构里的每个传感器，然后每次把该传感器上所有时间戳对应的数据交由该Chunk的pageWriter写入到其对应的两个输出流timeOut和valueOut的缓存中，并检查该Chunk的pageWriter的数据点or占用内存的大小情况，判断是否要开启一个新的page，若要开启新的page则往对应Chunk的ChunkWriterImpl的输出流pageBuffer缓存里写入该page的pageHeader和pageData（即pageWriter对象里输出流timeOut和valueOut的缓存数据），最后重置该pageWriter
   ...
   */
  private long calculateMemSizeForAllGroup() {  //获取该TsFileWriter最大占用的内存大小，即其所有ChunkGroupWriter占用的最大内存大小的总和
    long memTotalSize = 0;
    for (IChunkGroupWriter group : groupWriters.values()) {
      memTotalSize += group.updateMaxGroupMemSize();//获取该ChunkGroupWriter最大占用的内存大小，即其所有ChunkWriter占用的最大内存大小的总和
    }
    return memTotalSize;
  }
   ...
   * @throws IOException exception in IO
   */
  private boolean checkMemorySizeAndMayFlushChunks() throws IOException {//判断内存是否到达临界值而需要flush所有的ChunkGroups，若需要则将该TsFileWriter的所有ChunkGroupWriter里的所有ChunkWriter的缓存数据按序缓存到TsFileIOWriter的输出流缓存out里，最后将out输出流给flush到本地对应的TsFile文件。
    if (recordCount >= recordCountForNextMemCheck) {  //如果数据条数量到达指定数量上限（100），则
      long memSize = calculateMemSizeForAllGroup();  //获取该TsFileWriter最大占用的内存大小，即其所有ChunkGroupWriter占用的最大内存大小的总和
      assert memSize > 0;
   ...
   * @throws IOException exception in IO
   */
  public boolean flushAllChunkGroups() throws IOException {//将该TsFileWriter的所有ChunkGroupWriter里的所有ChunkWriter的缓存数据按序缓存到TsFileIOWriter的输出流缓存out里，最后将out输出流给flush到本地对应的TsFile文件。具体的做法如下：
                                                            // （1）遍历当前TsFileWriter的所有ChunkGroupWriter，将该ChunkGroupWriter的所有ChunkWriter对应的Chunk数据（ChunkHeader+ChunkData）给flush到TsFileIOWriter类对象里的输出缓存流out里。具体做法是遍历该ChunkGroupWriter的所有ChunkWriter做如下操作：首先封口当前page(即把当前Chunk的pageWriter输出流timeOut和valueOut的缓存数据写到该Chunk的ChunkWriterImpl的输出流pageBuffer缓存里,最后重置该pageWriter)，然后往TsFileIOWriter对象的TsFileOutput输出对象的输出流BufferedOutputStream的缓存数组里写入该Chunk的ChunkHeader,最后再写入当前Chunk的所有page数据（pageBuffer输出流的缓存数组内容）.
                                                            //（2）将该TsFile的TsFileIOWriter对象的输出缓存流TsFileOutput的内容给flush到本地对应TsFile文件
                                                            //（3）reset该TsFileWriter的相关属性，完成该次的flush操作
    if (recordCount > 0) {
      for (Map.Entry<String, IChunkGroupWriter> entry : groupWriters.entrySet()) {//遍历所有的groupWriter
        String deviceId = entry.getKey();
        IChunkGroupWriter groupWriter = entry.getValue();
        fileWriter.startChunkGroup(deviceId);// 开启一个新的设备ChunkGroup，并把该新的ChunkGroupHeader的内容（marker（为0）和设备ID）写入该TsFileIOWriter对象的TsFileOutput写入对象的输出流BufferedOutputStream的缓存数组里
        long pos = fileWriter.getPos();//获取当前TsFileIOWriter对象的输出缓存流out的长度，或者说缓存流out的缓存数组的指针位置
        long dataSize = groupWriter.flushToFileWriter(fileWriter);//将该ChunkGroupWriter的所有ChunkWriter对应的Chunk数据（ChunkHeader+ChunkData）给flush到TsFileIOWriter类对象里的输出缓存流out里，并返回总共flush的字节数。具体做法是遍历该ChunkGroupWriter的所有ChunkWriter做如下操作：首先封口当前page(即把当前Chunk的pageWriter输出流timeOut和valueOut的缓存数据写到该Chunk的ChunkWriterImpl的输出流pageBuffer缓存里,最后重置该pageWriter)，然后往TsFileIOWriter对象的TsFileOutput输出对象的输出流BufferedOutputStream的缓存数组里写入该Chunk的ChunkHeader,最后再写入当前Chunk的所有page数据（pageBuffer输出流的缓存数组内容）.
        ...
        fileWriter.endChunkGroup();// 结束currentChunkGroupDeviceId对应ChunkGroup，即把其ChunkGroupMeatadata元数据加入该写入类的chunkGroupMetadataList列表缓存里，并把该写入类的相关属性（设备ID和ChunkMetadataList清空），并将该TsFile的TsFileIOWriter对象的输出缓存流TsFileOutput的内容给flush到本地对应TsFile文件
      }
