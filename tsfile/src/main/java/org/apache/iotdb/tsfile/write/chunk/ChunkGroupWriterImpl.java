...
public class ChunkGroupWriterImpl implements IChunkGroupWriter {//TsFile文件里，每个设备ID应该对应一个ChunkGroupWriter类对象。ChunkGroupWriter类里存放着每个传感器ID对应的ChunkWriter类对象
  ...
  public void write(Tablet tablet) throws WriteProcessException {//依次遍历tablet结构里的每个传感器，然后每次把该传感器上所有时间戳对应的数据交由该Chunk的pageWriter写入到其对应的两个输出流timeOut和valueOut的缓存中，并检查该Chunk的pageWriter的数据点or占用内存的大小情况，判断是否要开启一个新的page，若要开启新的page则往对应Chunk的ChunkWriterImpl的输出流pageBuffer缓存里写入该page的pageHeader和pageData（即pageWriter对象里输出流timeOut和valueOut的缓存数据），最后重置该pageWriter
    ...
    for (int i = 0; i < timeseries.size(); i++) {//遍历每个传感器配置类对象
      ...
      if (dataType.equals(TSDataType.VECTOR)) { //如果数据类型是多元数据，则
        ...
        writeByDataType(tablet, measurementId, dataType, i);// 将给定的Tablet里的数据点数组交由该Chunk的pageWriter写入到其对应的两个输出流timeOut和valueOut的缓存中，并检查该Chunk的pageWriter的数据点or占用内存的大小情况，判断是否要开启一个新的page，若要开启新的page则往对应Chunk的ChunkWriterImpl的输出流pageBuffer缓存里写入该page的pageHeader和pageData（即pageWriter对象里输出流timeOut和valueOut的缓存数据），最后重置该pageWriter
      }
    }
  }
   ...
   */
  private void writeByDataType(// 将给定的Tablet里的数据点数组交由该Chunk的pageWriter写入到其对应的两个输出流timeOut和valueOut的缓存中，并检查该Chunk的pageWriter的数据点or占用内存的大小情况，判断是否要开启一个新的page，若要开启新的page则往对应Chunk的ChunkWriterImpl的输出流pageBuffer缓存里写入该page的pageHeader和pageData（即pageWriter对象里输出流timeOut和valueOut的缓存数据），最后重置该pageWriter
    ...
    switch (dataType) {
      case INT32:
        chunkWriters
            .get(measurementId)
            .write(tablet.timestamps, (int[]) tablet.values[index], batchSize);// 将给定的数据点数组交由该Chunk的pageWriter写入到其对应的两个输出流timeOut和valueOut的缓存中，并检查该Chunk的pageWriter的数据点or占用内存的大小情况，判断是否要开启一个新的page，若要开启新的page则往对应Chunk的ChunkWriterImpl的输出流pageBuffer缓存里写入该page的pageHeader和pageData（即pageWriter对象里输出流timeOut和valueOut的缓存数据），最后重置该pageWriter
        break;
      case INT64:
        chunkWriters
            .get(measurementId)
  ...
  public long flushToFileWriter(TsFileIOWriter fileWriter) throws IOException {//将该ChunkGroupWriter的所有ChunkWriter对应的Chunk数据（ChunkHeader+ChunkData）给flush到TsFileIOWriter类对象里的输出缓存流out里，并返回总共flush的字节数。具体做法是遍历该ChunkGroupWriter的所有ChunkWriter做如下操作：首先封口当前page(即把当前Chunk的pageWriter输出流timeOut和valueOut的缓存数据写到该Chunk的ChunkWriterImpl的输出流pageBuffer缓存里,最后重置该pageWriter)，然后往TsFileIOWriter对象的TsFileOutput输出对象的输出流BufferedOutputStream的缓存数组里写入该Chunk的ChunkHeader,最后再写入当前Chunk的所有page数据（pageBuffer输出流的缓存数组内容）.
    ...
    long currentChunkGroupSize = getCurrentChunkGroupSize();//获取当前ChunkGroup的字节大小，即其所有Chunk的字节大小(ChunkHeader+ChunkData，ChunkData即缓存pageBuffer）的总和
    for (IChunkWriter seriesWriter : chunkWriters.values()) {//遍历所有ChunkWriter
      seriesWriter.writeToFileWriter(fileWriter);// 首先封口当前page(即把当前Chunk的pageWriter输出流timeOut和valueOut的缓存数据写到该Chunk的ChunkWriterImpl的输出流pageBuffer缓存里,最后重置该pageWriter)，然后往TsFileIOWriter对象的TsFileOutput输出对象的输出流BufferedOutputStream的缓存数组里写入该Chunk的ChunkHeader,最后再写入当前Chunk的所有page数据（pageBuffer输出流的缓存数组内容）
    }
  ...
  @Override
  public long getCurrentChunkGroupSize() {//获取当前ChunkGroup的字节大小，即其所有Chunk的字节大小(ChunkHeader+ChunkData，ChunkData即缓存pageBuffer）的总和
    long size = 0;
    for (IChunkWriter writer : chunkWriters.values()) {
      size += writer.getCurrentChunkSize();//获取当前Chunk的字节大小，即ChunkHeader+ChunkData（缓存pageBuffer）的大小
    }
    return size;
  }

  /** seal all the chunks which may has un-sealed pages in force. */
  private void sealAllChunks() {// 关闭、封口该ChunkGroupWriter里所有ChunkWriter对应的当前page(即把当前Chunk的pageWriter输出流timeOut和valueOut的缓存数据写到该Chunk的ChunkWriterImpl的输出流pageBuffer缓存里,最后重置该pageWriter)
    for (IChunkWriter writer : chunkWriters.values()) {
      writer.sealCurrentPage();// 关闭、封口当前page(即把当前Chunk的pageWriter输出流timeOut和valueOut的缓存数据写到该Chunk的ChunkWriterImpl的输出流pageBuffer缓存里,最后重置该pageWriter)
    }
  }
